...
export function initWindowedXYZSelectable(inDatasets, opts = {}) {
  ...
  const d0 = _datasets[0];
  _F = d0?.positions?.length ?? 0;
  if (!_F) { console.error('plots: positions must be [F][M][3]'); return; }

  // Selector list comes ONLY from FreeMoCap (dataset 0)
  if (Array.isArray(d0.landmarks) && d0.landmarks.length) {
    _fmcNames = d0.landmarks.map(String);
  } else {
    // fallback to Marker i if FMC has no names
    const M0 = d0.positions[0]?.length ?? 0;
    _fmcNames = Array.from({ length: M0 }, (_, i) => `Marker ${i}`);
    for (let i = 0; i < _datasets.length; i++) {
      const ds = _datasets[i];
      const M = ds.positions[0]?.length ?? 0;
      for (let j = 0; j < Math.min(M0, M); j++) ds.nameToIndex.set(`Marker ${j}`, j);
    }
  }
